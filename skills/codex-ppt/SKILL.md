...
- Execute all phases automatically without stopping for confirmations or approvals. Proceed end-to-end from source content to final `.pptx` without waiting for user input.
- The visual style is always taken directly from the user's prompt. Do not offer style options or wait for style confirmation.
- Every remaining slide image job must be dispatched to a slide subagent whenever subagents are available.
...
- The selected image backend must stay fixed once chosen. Do not let subagents switch backend for convenience.
- Record how the first slide was generated and pass that exact method to every slide subagent.
...
2. Generate one sample slide to validate style and backend.
...
2. Plan the deck outline.
   ...
   - Draft slide roles and required source images, then immediately proceed to the next phase without waiting.

3. Apply the visual style from the prompt.
   - Before using files from `references/`, read `docs/outline-style-and-sample.md`.
   - Extract the style specified in the user's prompt. Do not ask for style confirmation. If no style is specified, choose the most appropriate style from `references/` and proceed.

4. Select the image backend.
   - Before generating any slide image, read `docs/backend-selection.md`.
   - Check whether a built-in image tool is callable, name the selected backend, and proceed immediately without waiting for confirmation.
...
5. Generate one sample slide to validate style and backend.
   - Before generating the sample slide, read `docs/outline-style-and-sample.md`.
   - Generate exactly one representative sample, record `sample_generation_method` in `deck_spec.json`, then immediately continue to full-deck generation without waiting for approval.
...
7. Prepare user-supplied assets.
   ...
   - Map required assets to slides and proceed with generation immediately.
...
11. Write speaker notes and assemble the PPT.
    ...
    - Ensure `slide_jobs.json` shows all generated slides as `recorded` before assembly. Repair any `pending` or `blocked` slides before proceeding.
...
Slide subagents are dispatched immediately after the first slide is generated whenever the runtime can spawn them. The main agent prepares jobs and records state; each worker handles exactly one `prompts/slide_XX.json` job and returns only selected image path, backend, and QA note.
...
- Every final slide image was generated by the confirmed backend and recorded through `record_slide_result.py`.
- `outline.md` reflects the deck outline.
